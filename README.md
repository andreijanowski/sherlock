...
- [styled-components](https://www.styled-components.com) - CSS-in-JS styling solution, preferred way to styling next.js apps
...
On the production some environments are needed for build, and some are used by node.js. We had some issues with that during first oauth deploy, so please be aware that some envs (used for build process) are located in CircleCi and loaded in [Dockerfile](https://github.com/netguru/sherlock/blob/master/docker/Dockerfile.production), and some (used by node.js server) are located in the [files](https://github.com/netguru/sherlock/blob/master/docker-compose-production.yml#L10) on production server.

## Setup

`yarn` to install dependencies

### Generating SSL certificate for localhost environment
...
Those commands should create `localhost.pem` and `localhost-key.pem` files inside the root directory.
...
`as an unauthenticated user` - the middleware grabs an access token from the server. This token is used to authorize the entire APP, and it gives us access to the public API
...
**CAVEAT:** Unfortunately because of the end of development and oauth migration, dynamic integration doesn't work as expected. After successful login user is redirected to master staging. To fix this, it is needed to
